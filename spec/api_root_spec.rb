...
describe "#{api_root_path} negative cases" do
  context 'with no basic auth' do
    context 'with valid accept header' do
      include_examples "unauthorized", get_no_auth(api_root_path, {'Accept' => TAXII_ACCEPT})
    end

    context 'with invalid accept header' do
      include_examples "not acceptable", get_no_auth(api_root_path)
      include_examples "not acceptable", get_no_auth(api_root_path, {'Accept' => 'invalid'})
    end
  end
